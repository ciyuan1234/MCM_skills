...
### Added
- `references/22-mechanism-pde.md`：A 类作战手册（SI 内核、附件只读、每问三件套、可接受模型七条、网格因时间尺度而变、效应分解、实验默认关闭）
- 配对验证 **V-PDE-1…12**（初值、零驱动、守恒、`r=0`、开尔文、网格+时间步、「各处达标」、模板回读、回归锁、效应分解）
- 反思银行 **RB-P01…P12**（机理/数值类，来源 2026A）
- 黄金测试集 `2026_A/`：结构检查点 + 已验证参考数字
- `plot-style.py` 灰阶/线型/标记栈（评委黑白打印）

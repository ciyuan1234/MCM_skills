...
作战流程见 `22-mechanism-pde.md`。阈值先向用户确认；赛题未给精度时只报收敛趋势，不声称精确。

| ID | 检查项 | 方法 | 通过标准 |
|---|---|---|---|
| V-PDE-1 | 初值 | `t=0` 场与题设/附件一致 | 逐点相符 |
| V-PDE-2 | 零驱动 | 边界与初值相同，积分若干步 | 状态不自发变化 |
| V-PDE-3 | 趋势 | 更热、更干边界 | 表面先变，内部滞后；禁止反向通量 |
| V-PDE-4 | 守恒/残差 | 内部变化 vs 累计边界通量；离散残差 | 相对误差通常应到 `1e-10` 量级；超 `1e-6` 必须解释 |
| V-PDE-5 | 奇点 | 柱/球坐标 `r=0` | 对称边界单独实现，无 `1/r` 直接代入 |
| V-PDE-6 | 温度单位 | 阿伦尼乌斯 / `\exp(-E/T)` | 内核用 K，不用 °C |
| V-PDE-7 | 空间收敛 | 至少两套网格，比较关键输出（终点或最慢点） | 场差或 `t_f` 差低于用户确认阈值 |
| V-PDE-8 | 时间步收敛 | 至少两套 `dt` | 同上；不得因 Picard 失败暗中放大 `dt` |
| V-PDE-9 | 「各处达标」 | 检查最慢位置（干燥问题通常是圆心） | 禁止用空间平均代替 |
| V-PDE-10 | 模板回读 | 写入官方 xlsx 后重新读关键单元格 | 与内存结果一致 |
| V-PDE-11 | 回归锁 | 后一问在前一问的设置下重跑 | 已确认结果零变化 |
| V-PDE-12 | 效应分解 | 两机制方向相反时 | 分别报告各机制贡献，不只报净值 |

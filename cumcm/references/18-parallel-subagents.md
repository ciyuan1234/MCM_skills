...
A 类常见 **Q1 ⊂ Q2 ⊂ Q3 ⊂ Q4**（同一对象逐步加机制）。这不满足「数据独立 + 模型独立」：

- 后一问必须能在前一问的设置下复现已确认结果（V-PDE-11）
- 共用 `h`、初值、附件插值器，并行会漂移
- 此类题目 **串行递进**，即使 AP 模式也不启动子问题 Agent

完整规则：`22-mechanism-pde.md` §6。

## 实验隔离（不是子问题并行）

机理题的并行对象是 **实验候选**，不是四个小问。默认不开实验；触发条件见 `22` §7。

获准后：

1. 每项实验一张任务卡 `docs/experiments/EXP-<编号>/brief.md`（唯一决策、候选 ≤3、指标、阈值、停止条件）
2. 从同一干净基线拉独立 worktree / 分支 `exp/<问题号>-<方法>-<编号>`
3. 每轮只改一个因素；禁止改主分支、附件、最终 `result*.xlsx`
4. 交付 `report.md` + 复跑命令；主 Agent 复跑后再建议，用户选择前不合并

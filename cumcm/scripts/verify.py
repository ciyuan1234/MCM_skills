...
def main():
    ...
    # 7. 决策日志完整性
    print("== 7. 决策日志 ==")
    dlog_path = os.path.join(workdir, "decision_log.json")
    if os.path.isfile(dlog_path):
        try:
            with open(dlog_path, encoding="utf-8") as f:
                dlog = json.load(f)
            report(PASS, f"decision_log.json 存在 (schema v{dlog.get('_schema_version', '?')})")
            # 检查已完成阶段的 hand_off.md
            stage_dirs = {
            ...
            for si, sdir in stage_dirs.items():
                ...
                if stage_info.get("status") == "completed":
                    hpath = os.path.join(workdir, sdir, "hand_off.md")
                    if os.path.isfile(hpath):
                        ...
                        try:
                            ...
                            if len(sections) >= 3:
                                report(PASS, f"  阶段 {si} hand_off.md 完整（三段式）")
                            else:
                                report(WARN, f"  阶段 {si} hand_off.md 缺少段落（需 3 个 '## What'，实际 {len(sections)} 个）")
                        except Exception:
                            report(WARN, f"  阶段 {si} hand_off.md 无法读取")
                    else:
                        report(WARN, f"  阶段 {si} 已完成但缺少 hand_off.md")
            # 检查时间预算
            budget = dlog.get("budget", {})
            remaining = budget.get("remaining_hours")
            if remaining is not None:
                if remaining <= 6:
                    report(WARN, f"  剩余时间 {remaining}h — 应处于锁定模式（<6h 只做写作/打包）")
                elif remaining <= 12:
                    report(WARN, f"  剩余时间 {remaining}h — 建议简化高级分析")
                else:
                    report(PASS, f"  剩余时间 {remaining}h — 正常")
            # 检查 decisions 数组
            decisions = dlog.get("decisions", [])
            if decisions:
                report(PASS, f"  已记录 {len(decisions)} 条决策")
            else:
                report(WARN, "  decisions 数组为空（建议记录关键决策及理由）")
        except Exception as e:
